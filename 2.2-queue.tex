...
\begin{frame}
  ...
  \begin{center}
    \begin{tikzpicture}
      % масивът
      \matrix (a) [widearray] {
        |(an+5)| \only<7| trans:0>{a_{n+5}} \&\&
        |(a0)| a_0 \& |(a1)| a_1 \& a_2 \& \ldots \& |(an)| a_n \&
        |(an+1)| \only<3-| trans:0>{a_{n+1}} \&
        |(an+2)| \only<4-| trans:0>{a_{n+2}} \&
        |(an+3)| \only<5-| trans:0>{a_{n+3}} \&
        |(an+4)| \only<6-| trans:0>{a_{n+4}} \\
      };

      % индекс на началото на опашката
      \pointerto[visible on=<1>]{a0}{\tt{front}}{below}

      % индекс на началото на опашката след премахване на елемент
      \pointerto[visible on=<2-| trans:0>]{a1}{\tt{front}}{below}

      % индекс на края на опашката
      \pointerto[visible on=<1-2>]{an}{\tt{back}}{below}
      ...
      \foreach \i in {1, ..., 5}{
        ...
        \pointerto[visible on=<\j| trans:0>]{an+\i}{\tt{back}}{below}
      };
    \end{tikzpicture}
  \end{center}
  \begin{itemize}
...
\begin{frame}<1-8>
  ...
  \begin{center}
    ...
    \begin{overlayarea}{\textwidth}{.4\textheight}
      \begin{tikzpicture}
        % веригата
        \doublecell[visible on=<1-7>]{a0}{a_0}
        \doublecell[right=2em of a0next]{a1}{a_1}
        \draw[pointer,visible on=<1-7>] (a0next.center) to (a1data);
        ...
        \nullptr[visible on=<1-2>]{annext}

        % указател към началото
        \pointerto[visible on=<1-5>]{a0.south}{\tt{front}}{below}

        % указател към края
        \pointerto[visible on=<1-3>]{an.south}{\tt{back}}{below}

        % създаване на нова клетка, сочеща към null
        \begin{scope}[visible on=<2-| trans:0>]
          \doublecell[right=2em of annext]{an+1}{a_{n+1}}
          \nullptr{an+1next}
        \end{scope}

        % първоначално се сочи от указател p, а после от back
        \pointerto[visible on=<2-| trans:0>]{an+1.south}{\tt{\alt<2-3>p{back}}}{below}

        % указателят на предишната последна клетка се насочва към новата последна
        \draw[pointer,visible on=<3-| trans:0>] (annext.center) to (an+1data);

        % нов указател към първата клетка
        \pointerto[visible on=<5-7| trans:0>]{a0}{\tt{p}}{above}

        % указателят front се насочва към втората клетка
        \pointerto[visible on=<6-| trans:0>]{a1}{\tt{front}}{below}

        % изтриване на първата клетка
        \draw (a0) node[cross,visible on=<7| trans:0>] {};
      \end{tikzpicture}
    \end{overlayarea}
  \end{center}
  \begin{itemize}
