...
\begin{frame}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \pointerto[visible on=<2->]{a1}{\tt{front}}{below}
      ...
      \foreach \i in {1, ..., 5}{
        ...
        \pointerto[visible on=<\j>]{an+\i}{\tt{back}}{below}
      };
    \end{tikzpicture}
  \end{center}
  \begin{itemize}
  ...
  \begin{center}
    ...
    \begin{overlayarea}{\textwidth}{.4\textheight}
      \begin{tikzpicture}
        ...
        \pointerto[visible on=<2->]{an+1.south}{\tt{\alt<2-3>p{back}}}{below}

        % указателят на предишната последна клетка се насочва към новата последна
        \draw[pointer,visible on=<3->] (annext.center) to (an+1data);

        % нов указател към първата клетка
        \pointerto[visible on=<5-7>]{a0}{\tt{p}}{above}

        % указателят front се насочва към втората клетка
        \pointerto[visible on=<6->]{a1}{\tt{front}}{below}

        % изтриване на първата клетка
        \draw (a0) node[cross,visible on=<7>] {};
      \end{tikzpicture}
    \end{overlayarea}
  \end{center}
  \begin{itemize}
  ...
  \begin{onlyenv}<trans:0>
    ...
    \begin{center}
      \begin{tabular}{|*{11}{c|}}
        ...
        \alt<4->5&\alt<5->3&\alt<6->6&\alt<7->1&\alt<8->2&\alt<3-8>\sent&\alt<5->5&\alt<6->6&\alt<7->3&\alt<8->2&\\
