  ...
  \begin{center}
    ...
    \begin{tikzpicture}
      ...
      \foreach \idx/\key in {1/Sylvia,6/Ivan,11/Marina} {
        \doublecell[below=3ex of a-1-\idx,inner sep=.5ex]{\key}{\tt{\key}}
        \draw[pointer] (a-1-\idx.center) to (\key);
      }
      \foreach \old/\new in {Ivan/Petar,Petar/Darina,Marina/Andrew} {
        \doublecell[below=3ex of \old next,inner sep=.5ex]{\new}{\tt{\new}}
        \draw[pointer] (\old next.center) to (\new);
      }
      \foreach \cell in {Sylvia,Darina,Andrew} { \nullptr{\cell next} }
    \end{tikzpicture}
  \end{center}
\end{frame}
  ...
  \begin{center}
    ...
    \begin{tikzpicture}
      \matrix [mtx,nodes={minimum width=4em}] {
        |(s)| Sylvia \& |(i)| Ivan \& \& \& |(p)| Peter \& |(d)| Darina \& \& |(a)| Andrew\\
      };
